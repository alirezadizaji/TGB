...
from ..hgcn.layers.hyplayers import HGCNConv, HypGRU, HGATConv
from ..hgcn.manifolds import PoincareBall
from .BaseModel import BaseModel


class HTGN(BaseModel):
    def __init__(self, args, device, heads, nb_window, aggregation, nout):
        super(HTGN, self).__init__(args, nb_window, nout, device)
        ...
        self.linear = nn.Linear(args.nfeat, nout)
        self.hidden_initial = torch.ones(args.num_nodes, nout).to(device)
        self.use_hta = args.use_hta
        if aggregation == 'deg':
            self.layer1 = HGCNConv(self.manifold, 2 * nout, 2 * nout, self.c[0], self.c[1],
                                   dropout=args.dropout)
            self.layer2 = HGCNConv(self.manifold, 2 * nout, nout, self.c[1], self.c[2], dropout=args.dropout)
        if aggregation == 'att':
            self.layer1 = HGATConv(self.manifold, 2 * nout, 2 * nout, self.c[0], self.c[1],
                                   heads=heads, dropout=args.dropout, att_dropout=args.dropout, concat=True)
            self.layer2 = HGATConv(self.manifold, 2 * nout * heads, nout, self.c[1], self.c[2],
                                   heads=heads, dropout=args.dropout, att_dropout=args.dropout, concat=False)
        self.gru = nn.GRUCell(nout, nout)

        self.nhid = nout
        self.nout = nout
        self.cat = True
        self.Q = Parameter(torch.ones((nout, nout)), requires_grad=True)
        self.r = Parameter(torch.ones((nout, 1)), requires_grad=True)
        self.num_window = nb_window
        self.init_hiddens()
