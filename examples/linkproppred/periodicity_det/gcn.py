"""
Dynamic Link Prediction with a GCN model with Early Stopping
"""
...
from ....tse import LinkPredictor, NodeFeatType, NodeFeatGenerator
...
def get_args():
    ...
    parser.add_argument('--in-channels', type=int, help='input channel dimension of GCN', default=100)
...
def train():
    r"""
    Training procedure for GCN model
...
@torch.no_grad()
def test(adj: torch.Tensor, neg_sampler: torch.Tensor, split_mode: torch.Tensor, out_2d: torch.Tensor, start_time: int):
    model['gnn'].eval()
...
for run_idx in range(NUM_RUNS):
    ...
    for epoch in range(1, NUM_EPOCH + 1):
        ...
        perf_metric_val, A = test(val_adj, neg_sampler, split_mode="val", out_2d=A, start_time=val_start_t)
    ...
    perf_metric_test, _ = test(test_adj, neg_sampler, split_mode="test", out_2d=A, start_time=test_start_t)
